...
use bridge::{
    ...
    utils::{num_blocks_per_network, decode_from_disk, encode_to_disk},
};
...
pub fn get_intermediate_variables_cached() -> BTreeMap<String, usize> {
    ...
    let intermediate_variables = if intermediate_variables_cache_path.exists() {
        decode_from_disk(&intermediate_variables_cache_path)
            .inspect_err(|e| {
                eprintln!(
    ...
    intermediate_variables.unwrap_or_else(|| {
        ...
        encode_to_disk(&intermediate_variables_cache_path, &intermediate_variables).unwrap();
